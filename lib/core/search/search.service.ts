...
import { deduplicateTikwmSearchResults } from "../track/dedupe";
...
async function runTikwmSearch(args: TikwmSearchArgs): Promise<SearchTikwmTracksPage> {
  ...
  // Deduplicate tracks by music_info.id, keeping highest scoring — stats preserved
  const tracks = deduplicateTikwmSearchResults(videos);
...
export const search = unstable_cache(
  async (args: TikwmSearchArgs): Promise<SearchTikwmTracksPage> => {
